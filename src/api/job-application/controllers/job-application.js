  ...
  ({ strapi }) => ({
    async create(ctx) {
      ...
      try {
        ...
        const emailParams = {
          Source: process.env.AWS_SES_FROM_EMAIL || "info@whizhack.com",
          Destination: {
            ToAddresses: [process.env.AWS_SES_TO_EMAIL || "hr@whizhack.com"],
          },
          Message: {
            Subject: {
              Data: `New Job Application - ${appliedFor}`,
              Charset: 'UTF-8'
            },
            Body: {
              Text: {
...
LinkedIn URL: ${linkedin}`,
        ...
        try {
          ...
          return ctx.send({
            ...
            warning: "HR team was not notified via email"
          });
        }
